...
Status: implemented. The desktop client now enters through thin `gui.py`, with game flow in `src/ui/game_controller.py`, input mapping in `src/ui/input_controller.py`, rendering in `src/ui/pygame_renderer.py`, and the pygame loop in `src/ui/pygame_app.py`. PVP and PVE both attempt model-backed first-player vs. second-player win-rate prediction during gameplay; when model loading or inference fails, the bar locks to 50:50 and displays `Invalid`. When inference is available, the current human player's top-three legal policy probabilities replace the center dot of the existing legal-move breathing highlights. Their standard-color outer rings keep breathing below a cell-safe maximum; the first choice's text uses bright gray and the other two use light gray. PVE hides all legal-position and probability highlights during the AI's turn and uses a first-legal-move fallback when AI move inference cannot run. PvE names its assigned sides `YOU` and `COMPUTER` across the desktop UI, and its result dialog uses the grammatically correct `YOU WIN`. The sidebar includes controls to toggle the win projection and move suggestions, as well as an immediate current-score match settlement action.
